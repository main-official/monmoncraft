...
class WorldRenderer {
  ...
  // Builds every chunk in the world, but does so progressively: a small
  // ring of chunks around (centerX, centerZ) — typically the player's spawn
  // point — is built synchronously so the game can start right away, then
  // the remaining chunks stream in during idle time in small batches. This
  // matters once WORLD_SIZE gets large, since building every chunk in one
  // synchronous pass would block the main thread for many seconds and
  // freeze the tab. onReady fires once the spawn area is ready to play.
  buildAllChunks(centerX = 0, centerZ = 0, onReady = null) {
    const [cx0, cz0] = this.world.chunkCoord(centerX, centerZ);
    const coords = [];
    for (let cx = 0; cx < CHUNKS_PER_SIDE; cx++) {
      for (let cz = 0; cz < CHUNKS_PER_SIDE; cz++) {
        const dx = cx - cx0, dz = cz - cz0;
        coords.push({ cx, cz, dist: dx * dx + dz * dz });
      }
    }
    coords.sort((a, b) => a.dist - b.dist);

    const SPAWN_RADIUS_CHUNKS = 3;
    const spawnDistSq = SPAWN_RADIUS_CHUNKS * SPAWN_RADIUS_CHUNKS;
    const immediate = coords.filter(c => c.dist <= spawnDistSq);
    const deferred = coords.filter(c => c.dist > spawnDistSq);

    for (const { cx, cz } of immediate) this.rebuildChunk(cx, cz);
    if (onReady) onReady();
    ...
    const step = () => {
      const end = Math.min(i + BATCH_SIZE, deferred.length);
      for (; i < end; i++) this.rebuildChunk(deferred[i].cx, deferred[i].cz);
      if (i < deferred.length) schedule(step);
    };
    schedule(step);
  }
