...
class Multiplayer {
  ...
  sendBlockChange(x, y, z, id) {
    ...
    const key = `${x}_${y}_${z}`;
    // Persist BLOCK.AIR (0) as an explicit value rather than deleting the
    // key. Firebase treats a missing key as "no override" (falls back to
    // the generated terrain), so removing the key on a break would make
    // broken blocks reappear on reload/rejoin. Only a genuinely missing id
    // (null/undefined) should clear the record.
    if (id == null) {
